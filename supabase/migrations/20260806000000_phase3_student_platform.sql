...
DROP POLICY IF EXISTS "student_subscriptions_user" ON public.student_subscriptions;
...
DROP POLICY IF EXISTS "slot_bookings_user" ON public.slot_bookings;
...
DROP POLICY IF EXISTS "payment_history_user" ON public.payment_history;
CREATE POLICY "payment_history_user" ON public.payment_history
  FOR SELECT USING (auth.uid() = user_id);

-- subscription_history: Users can see their own
DROP POLICY IF EXISTS "subscription_history_user" ON public.subscription_history;
...
DROP POLICY IF EXISTS "onboarding_progress_user" ON public.student_onboarding_progress;
