...
export function useStudentAnalytics(userId?: string) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const learningHours = completedSessions.reduce(
        (acc: number, s: any) => acc + (s.duration_mins ?? 0) / 60,
        0,
      );
      const sessionsCompleted = completedSessions.length;
      const resourcesStudied = resources.length;
      const xpTotal =
        (xp ?? []).reduce((acc: number, row: any) => acc + (row.points ?? 0), 0) +
      ...
      const totalRelevant =
        sessionsArr.filter((s: any) => !["cancelled", "rejected"].includes(s.status)).length || 1;
...
export function useMentorAnalytics(mentorId?: string) {
  return useQuery({
    ...
    enabled: !!mentorId,
    queryFn: async () => {
      const [{ data: sessions = [] }, { data: reviews = [] }, { data: resources = [] }] =
        await Promise.all([
          client
            .from("sessions")
            .select("id,scheduled_time,duration_mins,status,student_id")
      ...
      const todays = sessionsArr2.filter(
        (s: any) => new Date(s.scheduled_time).toDateString() === today,
      );
      const monthly = sessionsArr2.filter(
        (s: any) => new Date(s.scheduled_time).getMonth() === new Date().getMonth(),
      );
      const revenue = monthly.reduce(
        (acc: number, s: any) => acc + ((s.duration_mins ?? 0) / 60) * 0,
        0,
      ); // placeholder
      const avgRating = reviewsArr.length
        ? reviewsArr.reduce((a: number, r: any) => a + r.rating, 0) / reviewsArr.length
        : 0;
      const students = new Set(sessionsArr2.map((s: any) => s.student_id)).size;
      const repeatStudents = (() => {
        const counts: Record<string, number> = {};
        sessionsArr2.forEach((s: any) => {
      ...
      const completionRate = sessionsArr2.length
        ? Math.round(
            (sessionsArr2.filter((s: any) => s.status === "completed").length /
              sessionsArr2.length) *
              100,
          )
        : 0;
