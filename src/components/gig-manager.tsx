...
const CATEGORIES = [
  "Conversation", "Grammar", "Business", "Exam Prep", "Pronunciation",
  "Writing", "Reading", "Culture", "Academic", "Travel", "Kids",
];
...
export function GigManager({ mentorId }: GigManagerProps) {
  const { gigs, isLoading, createGig, updateGig, archiveGig, deleteGig, toggleActive, uploadCoverImage, defaultGigForm } = useGigs(mentorId);
  ...
  function openEdit(gig: Tables<"gigs">) {
    ...
    setForm({
      ...
      learning_outcomes: Array.isArray(gig.learning_outcomes) ? gig.learning_outcomes as string[] : [],
          ...
          <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                {form.cover_image && (
                  <div className="relative h-32 w-full overflow-hidden rounded-lg mb-2">
                    ...
                    <Button size="sm" variant="ghost" className="absolute right-2 top-2" onClick={() => setForm((p) => ({ ...p, cover_image: null }))}>
                      Remove
                    </Button>
                  </div>
                )}
                <label className="flex cursor-pointer items-center gap-2 rounded-md border border-dashed p-3 text-sm text-muted-foreground hover:border-primary/50 transition-colors">
                  ...
                  <input type="file" accept="image/*" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) handleUploadCover(f); e.target.value = ""; }} />
                </label>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label>Title *</Label>
                  <Input value={form.title} onChange={(e) => setForm((p) => ({ ...p, title: e.target.value }))} placeholder="e.g., Conversational Spanish" />
                </div>
                <div className="space-y-2">
                  <Label>Price ($) *</Label>
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                ...
                <div className="space-y-2">
                  ...
                  <select className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm" value={form.language} onChange={(e) => setForm((p) => ({ ...p, language: e.target.value }))}>
                    {LANGUAGES.map((l) => <option key={l.code} value={l.code}>{l.emoji} {l.name}</option>)}
                  </select>
                </div>
                <div className="space-y-2">
                  <Label>Level</Label>
                  <select className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm" value={form.level} onChange={(e) => setForm((p) => ({ ...p, level: e.target.value }))}>
              ...
              <div className="space-y-2">
                ...
                <select className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm" value={form.category} onChange={(e) => setForm((p) => ({ ...p, category: e.target.value }))}>
              ...
              <div className="space-y-2">
                ...
                <Textarea rows={3} value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} placeholder="Describe what this gig offers..." />
              </div>

              <div className="space-y-2">
                <Label>What's included</Label>
                <AddItemInput onAdd={(v) => updateArrayField("whats_included", v)} placeholder="e.g., Personalized feedback" />
              ...
              <div className="space-y-2">
                ...
                <AddItemInput onAdd={(v) => updateArrayField("learning_outcomes", v)} placeholder="e.g., Hold a 10-min conversation" />
                <div className="flex flex-wrap gap-1.5 mt-1">
                  {form.learning_outcomes.map((item, i) => (
                    <Badge key={i} variant="outline" className="gap-1">
                      {item}
                      <button onClick={() => removeArrayItem("learning_outcomes", i)}>&times;</button>
                    </Badge>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input value={form.prerequisites} onChange={(e) => setForm((p) => ({ ...p, prerequisites: e.target.value }))} placeholder="e.g., Basic knowledge of the language" />
              </div>

              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                <div className="flex items-center justify-between rounded-lg border p-3">
                  <Label className="text-xs cursor-pointer">Homework</Label>
                  <Switch checked={form.homework_included} onCheckedChange={(v) => setForm((p) => ({ ...p, homework_included: v }))} />
                </div>
                <div className="flex items-center justify-between rounded-lg border p-3">
                  <Label className="text-xs cursor-pointer">Recording</Label>
                  <Switch checked={form.recording_included} onCheckedChange={(v) => setForm((p) => ({ ...p, recording_included: v }))} />
                </div>
                <div className="flex items-center justify-between rounded-lg border p-3">
                  <Label className="text-xs cursor-pointer">Certificate</Label>
                  <Switch checked={form.certificate_included} onCheckedChange={(v) => setForm((p) => ({ ...p, certificate_included: v }))} />
                </div>
                <div className="flex items-center justify-between rounded-lg border p-3">
              ...
              <Button onClick={handleSave} disabled={createGig.isPending || updateGig.isPending} className="w-full">
                {(createGig.isPending || updateGig.isPending) && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {gigs.map((gig) => (
            <GigCard
              ...
              showActions
              onEdit={openEdit}
              onArchive={(id) => { archiveGig.mutate(id); toast.success("Gig archived"); }}
              onDelete={(id) => { deleteGig.mutate(id); toast.success("Gig deleted"); }}
