          ...
          <Card>
            <CardContent>
              ...
              <div className="text-2xl font-semibold">
                {isLoading ? "—" : (data?.learningHours ?? 0).toFixed(1)}
              </div>
            </CardContent>
          </Card>
          <Card>
            <CardContent>
              ...
              <div className="text-2xl font-semibold">
                {isLoading ? "—" : (data?.sessionsCompleted ?? 0)}
              </div>
            </CardContent>
          </Card>
          <Card>
            <CardContent>
              ...
              <div className="text-2xl font-semibold">
                {isLoading ? "—" : `${data?.homeworkCompletionPct ?? 0}%`}
              </div>
            </CardContent>
          </Card>
        </div>
            ...
            <CardContent style={{ height: 240 }}>
              {isLoading ? (
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart
                    data={(data?.weeklyActivity ?? []).map((d: any, i: number) => ({
                      name: `Day ${i + 1}`,
                      value: d.count,
                    }))}
                  >
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-2">
                ...
                <div className="text-2xl font-semibold">
                  {isLoading ? "—" : (data?.xpTotal ?? 0)}
                </div>
                <div className="text-sm text-muted-foreground">
                  Current streak: {isLoading ? "—" : data?.currentStreak}
                </div>
                <div className="text-sm text-muted-foreground">
                  Longest streak: {isLoading ? "—" : data?.longestStreak}
                </div>
              </div>
            </CardContent>
          </Card>
