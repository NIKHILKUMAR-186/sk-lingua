    ...
    <AppShell variant="student">
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        {sessionResources.length > 0 ? (
          <section className="space-y-3 p-3 rounded-lg border border-muted-foreground/10 bg-muted/5">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {sessionResources.map((resource) => (
                <Card key={resource.id}><CardContent>
                  <div className="flex items-start p-3 justify-between gap-3">
                    <div>
                  ...
                  <div className="mt-4 flex items-center justify-between text-xs text-muted-foreground">
                    ...
                    <a href={resource.storage_url || resource.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-primary underline">
                      <Download className="h-5 w-5" /> Open
                    </a>
                  </div>
                </CardContent></Card>
              ))}
                ...
                return (
                  <Card key={resource.id} className="transition hover:shadow-soft">
                    <CardContent>
                      ...
                      {resource.description ? <p className="mt-3 text-sm border bg-muted/5 p-3 text-muted-foreground">{resource.description}</p> : null}
                    </CardContent>
                  </Card>
                );
              })}
