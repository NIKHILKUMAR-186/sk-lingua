...
function StreakPage() {
  ...
  const { data: streak } = useQuery({
    queryKey: ["streak-full", auth?.user?.id], enabled: !!auth?.user,
    queryFn: async () => (await supabase.from("streak_points").select("*").eq("user_id", auth!.user!.id).maybeSingle()).data,
  });
  const { data: leaderboard = [] } = useQuery({
    queryKey: ["leaderboard"],
    queryFn: async () => {
      const { data } = await supabase.from("streak_points").select("user_id, total_points, current_streak").order("total_points", { ascending: false }).limit(10);
  ...
  return (
    <AppShell variant="student">
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <Card><CardHeader><CardTitle>Last 12 weeks</CardTitle></CardHeader><CardContent>
          <div className="grid grid-cols-12 gap-1">
            {grid.map((active, i) => (
              <div key={i} className={`aspect-square rounded ${active ? "bg-flame" : "bg-muted"}`} />
            ))}
          </div>
        </CardContent></Card>
        ...
        <Card><CardHeader><CardTitle>Badges</CardTitle></CardHeader><CardContent>
          <div className="grid gap-3 sm:grid-cols-4">
            {BADGES.map((b) => {
              ...
              return (
                <div key={b.name} className={`rounded-lg border p-4 text-center ${unlocked ? "border-primary bg-primary/5" : "opacity-50"}`}>
                  <Trophy className={`mx-auto h-6 w-6 ${unlocked ? "text-warning" : "text-muted-foreground"}`} />
        ...
        <Card><CardHeader><CardTitle>Leaderboard</CardTitle></CardHeader><CardContent className="space-y-2">
          {leaderboard.map((l, i) => (
            <div key={l.user_id} className="flex items-center justify-between rounded-lg p-2 hover:bg-muted">
              <div className="flex items-center gap-3">
                <span className="w-6 text-center text-sm font-mono text-muted-foreground">{i + 1}</span>
                <div className="h-8 w-8 overflow-hidden rounded-full bg-muted">
                  {l.profile?.avatar_url ? <img src={l.profile.avatar_url} alt="" className="h-full w-full object-cover" /> : null}
                </div>
