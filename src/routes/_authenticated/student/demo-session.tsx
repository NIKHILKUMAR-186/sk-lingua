...
function DemoSessionPage() {
  ...
  async function handlePaymentSubmit(data: any) {
    setLoading(true);
    try {
      const basePrice = 9.0; // ₹9 demo price
      const { taxAmount, finalAmount } = calculatePaymentSummary(basePrice);
      ...
      <div className="mx-auto max-w-3xl space-y-6 py-6">
        ...
        <div className="grid gap-6 md:grid-cols-3">
          <div className="md:col-span-2">
            {step === "info" && (
              <DemoInfoPage onGetStarted={() => setStep("booking")} loading={loading} />
            )}
            ...
            {step === "confirmation" && (
              <div className="space-y-6">
                <div className="rounded-3xl border-2 border-green-200 bg-green-50 p-8 text-center">
                  ...
                  <h1 className="mt-4 text-2xl font-bold text-green-900">Demo Session Booked!</h1>
            ...
            <div>
              <div className="sticky top-6">
                <PaymentSummary
                  ...
                  items={[{ label: "Demo Session (30 min)", value: `₹${demoPrice}` }]}
                />
              </div>
            </div>
          )}
