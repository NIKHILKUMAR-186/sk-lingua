...
function MentorDashboard() {
  ...
  const { data: gigs = [], isLoading: gigsLoading } = useQuery({
    ...
    queryFn: async () => (await supabase.from("gigs").select("*").eq("mentor_id", uid!).eq("is_archived", false).order("created_at", { ascending: false })).data ?? [],
  });
  ...
  const isLoading = sessionsLoading || mpLoading || gigsLoading || reviewsLoading || resourcesLoading;
      ...
      <div className="mx-auto max-w-6xl space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              {isLoading ? (
                ...
                <EmptyState icon={Calendar} title="No pending requests" description="You'll see booking requests here." />
              ) : (
          ...
          <Card>
            ...
            <CardContent>
              {isLoading ? (
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">${Number(topGig.price).toFixed(0)}/session • {topGig.duration_mins} min</div>
                    </div>
                  </div>
                ...
                <EmptyState icon={PackageOpen} title="No gigs yet" description="Create your first gig to start receiving bookings." actionLabel="Create gig" onAction={() => window.location.href = "/mentor/profile"} />
              )}
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {isLoading ? (
                ...
                <div className="text-sm text-muted-foreground text-center py-4">No reviews yet.</div>
              ) : reviews.map((review) => (
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Button variant="ghost" size="sm" asChild><Link to="/mentor/resources">Manage <ArrowRight className="ml-1 h-3 w-3" /></Link></Button>
            </CardHeader>
            <CardContent className="space-y-3">
              {isLoading ? (
                <ListSkeleton items={3} />
                ...
                <div key={resource.id} className="flex items-center justify-between rounded-lg border p-3">
                  <div><div className="text-sm font-medium">{resource.title}</div><div className="text-xs text-muted-foreground">{resource.visibility}</div></div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <Button variant="ghost" size="sm" asChild><Link to="/mentor/sessions">See all <ArrowRight className="ml-1 h-3 w-3" /></Link></Button>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              <ListSkeleton items={3} />
            ) : upcoming.length === 0 ? (
              <EmptyState icon={Video} title="No upcoming sessions" description="Once a student books, you'll see it here." />
            ) : (
              <div className="space-y-2">{upcoming.slice(0, 5).map((s) => (
                <div key={s.id} className="flex items-center justify-between rounded-lg border p-3">
                  <div><div className="text-sm font-medium">{new Date(s.scheduled_time).toLocaleString()}</div><div className="text-xs text-muted-foreground">{s.duration_mins} min</div></div>
                  <Button size="sm" variant="outline" asChild><Link to="/mentor/session/$id" params={{ id: s.id }}>Open</Link></Button>
                </div>
              ))}</div>
            )}
          </CardContent>
