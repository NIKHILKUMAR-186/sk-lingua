...
import { CalendarDays, CheckCircle2, XCircle, Video, Clock, MessageSquare, User } from "lucide-react";
...
function MentorCalendar() {
  ...
  const { data: requests = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase.from("sessions").select("*").eq("mentor_id", uid).order("scheduled_time");
      ...
      const gigIds: string[] = [];
      data.forEach(d => { if (d.gig_id) gigIds.push(d.gig_id); });
      const { data: gigs } = await supabase.from("gigs").select("id, title").in("id", gigIds);
      const gigMap = new Map((gigs ?? []).map(g => [g.id, g]));

      return data.map(d => ({ ...d, student: byId.get(d.student_id), gig: d.gig_id ? gigMap.get(d.gig_id) : null }));
    },
  });
  ...
  async function updateStatus(id: string, status: "accepted" | "rejected" | "completed") {
    ...
    if (error) toast.error(error.message);
      ...
      toast.success(status === "accepted" ? "Booking accepted" : status === "rejected" ? "Booking rejected" : "Marked complete");
        ...
        <Tabs defaultValue="pending">
          ...
          <TabsContent value="pending" className="mt-4 space-y-3">
            {isLoading ? (
              ...
              <EmptyState icon={CalendarDays} title="No pending requests" description="When a student books a session, you'll see it here." />
            ) : (
              pending.map((r) => (
                        ...
                        <div className="flex items-start gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                              <span className="flex items-center gap-1"><CalendarDays className="h-3 w-3" />{new Date(r.scheduled_time).toLocaleString()}</span>
                              <span className="flex items-center gap-1"><Clock className="h-3 w-3" />{r.duration_mins} min</span>
                              {r.gig && <Badge variant="outline" className="text-[10px]">{r.gig.title}</Badge>}
                            </div>
                            {r.student_message && (
                              <div className="mt-2 flex items-start gap-1.5 rounded-lg bg-muted/50 p-2">
                                <MessageSquare className="mt-0.5 h-3 w-3 shrink-0 text-muted-foreground" />
                                <p className="text-xs text-muted-foreground italic">"{r.student_message}"</p>
                              </div>
                            )}
                          </div>
                        </div>
          ...
          <TabsContent value="upcoming" className="mt-4 space-y-3">
            {isLoading ? (
              ...
              <EmptyState icon={Video} title="No upcoming sessions" description="Accepted sessions will appear here." />
            ) : (
              upcoming.map((r) => (
                <motion.div key={r.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                  <Card>
                        ...
                        <div className="flex gap-2">
                          {r.video_call_link && (
                            <Button size="sm" asChild>
                              <a href={r.video_call_link} target="_blank" rel="noreferrer"><Video className="mr-1 h-4 w-4" /> Join</a>
                            </Button>
                          )}
          ...
          <TabsContent value="past" className="mt-4 space-y-3">
            {isLoading ? (
              ...
              <EmptyState icon={Clock} title="No past sessions" description="Completed and cancelled sessions appear here." />
            ) : (
              past.map((r) => (
                <Card key={r.id}>
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between gap-4">
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant={r.status === "completed" ? "outline" : "secondary"}>{r.status}</Badge>
                        </div>
