      ...
      <div className="mx-auto max-w-4xl space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Keep session paperwork and homework attached to each lesson.
          </p>
        </div>
        {activeSessionId ? (
          <Card>
            ...
            <CardContent>
              <ResourceUpload
                ...
                sessions={sessions
                  ...
                  .map((session) => ({
                    id: session.id,
                    label: `${new Date(session.scheduled_time).toLocaleString()} • ${session.student_id?.slice(0, 8) ?? "student"}`,
                  }))}
        ...
        {sessions.length === 0 ? (
          ...
          sessions.map((s) => {
            const sessionResources = sharedResources.filter(
              (resource) => resource.session_id === s.id,
            );
            return (
              <Card key={s.id}>
                <CardContent className="space-y-3 p-4">
                  ...
                  <div className="rounded-lg border bg-muted/20 p-3 text-sm">
                    ...
                    {sessionResources.length === 0 ? (
                      <div className="mt-1 text-muted-foreground">
                        No materials shared for this lesson yet.
                      </div>
                    ) : (
                      <div className="mt-2 flex flex-wrap gap-2">
                        {sessionResources.slice(0, 3).map((resource) => (
                  ...
                  <div className="rounded-lg border bg-background/70 p-3 text-sm">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <Button size="sm" variant="outline" onClick={() => saveNotes(s.id)}>
                        Save
                      </Button>
                    </div>
                    <Textarea
                      ...
                      onChange={(event) =>
                        setNotesDrafts((prev) => ({ ...prev, [s.id]: event.target.value }))
                      }
                    />
                  </div>
                </CardContent>
