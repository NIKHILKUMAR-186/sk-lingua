  ...
  async function updateStatus(id: string, status: string) {
    try {
      ...
      if (status === "approved") {
        ...
        await import("@/lib/mentorApplications").then(
          async (mod) => await mod.updateApplicationStatus(id, status, auth!.user!.id),
        );
              ...
              <Card key={app.id}>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button size="sm" variant="ghost" onClick={() => scheduleInterviewFor(app.id)}>
                      Schedule interview
                    </Button>
                    <Button
                      size="sm"
                      ...
                      onClick={async () => {
                        try {
                          ...
                          const { error: roleErr } = await supabase
                            .from("user_roles")
                            .upsert([{ user_id: app.user_id, role: "mentor" }], {
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        if (!note) return;
                        try {
                          const res = await fetch("/api/admin/add-admin-note", {
                            method: "POST",
                            headers: { "Content-Type": "application/json" },
                            body: JSON.stringify({
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        if (!app.user_id) {
                          toast.error("No user associated with application");
                          return;
                        }
                        try {
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        // toggle active state
                        if (!app.user_id) {
                          toast.error("No user associated");
                          return;
                        }
                        try {
                          const action = app.is_active ? "suspend" : "activate";
                          const res = await fetch("/api/admin/toggle-mentor-status", {
                            method: "POST",
                            headers: { "Content-Type": "application/json" },
                            body: JSON.stringify({ userId: app.user_id, action }),
                          });
                          const data = await res.json();
                    ...
                    >
                      {app.is_active ? "Suspend" : "Activate"}
                    </Button>
                    <Button
                      size="sm"
                      variant="outline"
                      ...
                      onClick={async () => {
                        try {
                          const { error } = await supabase
                            ...
                            .update({ is_active: !(app.is_active ?? false) })
