...
export async function POST(req: Request) {
  try {
    ...
    if (action === "accept") {
      ...
      if (error) {
        return new Response(JSON.stringify({ error: error.message || "Failed to accept" }), { status: 409 });
      }

      const result = data as { success: boolean; error?: string; booking_id?: string };

      if (!result.success) {
        return new Response(JSON.stringify({ error: result.error || "Failed to accept" }), { status: 409 });
      }
      ...
      return new Response(JSON.stringify({ ok: true, booking_id: result.booking_id }), { status: 200 });
      ...
      if (error) {
        return new Response(JSON.stringify({ error: error.message || "Failed to decline" }), { status: 409 });
      }

      const result = data as { success: boolean; error?: string };
      if (!result.success) {
        return new Response(JSON.stringify({ error: result.error || "Failed to decline" }), { status: 409 });
      }
