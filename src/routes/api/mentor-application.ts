...
export const Route = createFileRoute("/api/mentor-application")({
  server: {
    handlers: {
      POST: async ({ request }) => {
  try {
    ...
    for (const k of required) {
      if (!payload[k])
        return new Response(JSON.stringify({ error: `missing ${k}` }), { status: 400 });
    }
    ...
    const { error: uploadErr } = await admin.storage
      .from("resources")
      .upload(path, base64ToUint8Array(resume.contentBase64), {
        contentType: resume.fileType || "application/pdf",
        upsert: false,
      });
    ...
    await admin.from("mentor_application_status_history").insert([
      {
        ...
        changed_by: null,
        notes: "Submitted via public application",
      },
    ]);
