...
export async function POST({ request }: { request: Request }) {
  try {
    ...
    if (!userId || !action) return new Response(JSON.stringify({ error: 'missing fields' }), { status: 400 });
    ...
    await admin.from('audit_logs').insert([{ actor_id: null, scope: 'mentor_profiles', action, details: { user_id: userId } }]);
